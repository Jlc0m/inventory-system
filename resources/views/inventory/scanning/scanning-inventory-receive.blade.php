            ...
            <div class="row">
                <div class="col-12">
                    <div class="card card-info">
                        ...
                        <form action="{{ route('scan-inventories-receive-post') }}" method="POST" class="p-2">
                            @csrf
                            @method('POST')
                            @if (session('status'))
                            ...
                            <div class="p-2">
                                ...
                                <label class="">Выберите город получения - *</label>
                                ...
                                <label class="mt-2">Выберите офис получения - *</label>
                                ...
                                <label class="mt-2">Выберите склад получения - *</label>
                                <select name="recipient_stock_id" class="form-control select2 col-md-4">
                                    <option value="">Выберите склад</option>
                                    @foreach ($user->offices as $office)
                                        @foreach ($office->stocks as $stock)
                                        <option value="{{ $stock->id }}">{{ $stock->name }}</option>
                                        @endforeach
                                    @endforeach
                                </select>

                                <label class="mt-2">Выберите транзакцию получения - *</label>
                                <select name="receive_transaction_id" class="form-control select2 col-md-4">
                                    <option value="">Выберите транзацию</option>
                                    @foreach ($transactionsReceive as $transactionReceive)
                                        <option value="{{ $transactionReceive->id }}">{{ $transactionReceive->id }}</option>
                                    @endforeach
                                </select>
